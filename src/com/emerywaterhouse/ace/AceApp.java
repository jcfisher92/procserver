/**
 * File: AceApp.java
 * Description: Process server application that handles the processing of data between Emery and Ace.
 * 
 * Author: Jeff Fisher, Eric Brownewell
 * 
...
public class AceApp extends ProcessApplication implements ConfLoader 
{
   private static final String confFileName = "aceproc.xml";
   public static final String exchange = "acecomm";
   private ArrayList<AceJobProc> m_JobProcs;
   ...
   public AceApp() 
   {
      ...
      m_JobProcs = new ArrayList<AceJobProc>();
      loadConf();
   ...
   /**
    * Implements the JMX addAceArticle method.  Calls the DataModule class to add a new
    *    Ace article to the database.
    *    
    * @param xmlRec The XML record sent by ace.
    * @return 0 = failed, non 0 is success.
    */
   public synchronized int processData(byte[] data) 
   {
      int result = 0;
      ArticleFeed af = new ArticleFeed(this, "jmxdp");
      ...
      try {
         ...
         af.processData();
         af.cleanup();
         result = af.getProcessResult();
      }
      
      catch ( Exception ex ) {
         ProcessServer.log.error("[AceApp]#ArticleFeed", ex);
      }
   ...
   /**
    * Adds a JobProc reference to the list of running processors.
    *
    * @param jobProc The running BodProcessor instance.
    */
   public synchronized void addJobProc(AceJobProc jobProc) 
   {
      if ( jobProc != null ) {
    ...
    *
    * Pulls messages from the topic and creates processing objects to deal with them.
    */
   @Override
    ...
    * @return
    */
   public AceJobProc getProcessor(String className) 
   {
      AceJobProc proc = null;
      Class<?> c = null;
      Object obj = null;
      ...
      if ( className != null && className.length() > 0 ) {
         try {
            ...
            if ( c != null ) {
               ...
               if ( obj instanceof AceJobProc )
                  proc = (AceJobProc) obj;
               else
                  ProcessServer.log.error("[AceApp]" + className + " is not an instance of the class AceJobProc");
            } 
            else
               ProcessServer.log.error("[AceApp] unable to instantiate " + className);
         }

         catch ( Exception ex ) {
            ProcessServer.log.error("[AceApp]", ex);
         }
    ...
    *
    * @param jobProc  The AceJobProc to remove.
    */
   public synchronized void remJobProc(AceJobProc jobProc) 
   {
      if ( jobProc != null ) {
         synchronized ( m_JobProcs ) {
            for ( int i = 0; i < m_JobProcs.size(); i++ ) {
         ...
         synchronized ( m_JobProcs ) {
            for (int i = 0; i < objList.getLength(); i++) {
               ...
               if (objNode.getNodeType() == Node.ELEMENT_NODE) {
                  ...
                  //
                  // Check to see if this job is already in the list of jobs.
                  // This conf file can be updated at any time so we don't want to have
                  // duplicates if it changes. We also don't want to clear everything out.
    ...
    * 
    * @param id The job's internal ID number.
    * 
   ...
   public boolean startJob(long id) 
   {
      ...
      Iterator<AceJobProc> iter = m_JobProcs.iterator();
      AceJobProc job = null;
   ...
   public void stop() 
   {
      ...
      while ( procCount > 0 ) {
         try {
            ...
            ProcessServer.log.warn("[AceApp] " + ex.getMessage());
         }
      }
   }
