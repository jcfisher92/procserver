...
public class AceJobProc implements Runnable 
{
   private static long startId = 1;
   
   protected AceApp m_App;          // Pointer to the process application instance.
   protected String m_CurProcStatus;// Current status message
   protected byte[] m_Data;         // Whatever data was pulled off of the queue and needs to be processed.    
   ...
   protected long m_MaxRunTime;     // The maximum amount of time the job should run without throwing an error.
   protected int m_MaxStartHour;    // The latest time the process can start running.
   protected int m_MinStartHour;    // The minimum hour to start processing.
   protected String m_Name;         // The name of the processor.
   protected int m_ProcResult;      // The result of any processing that needs to be shared.
   protected long m_RunTime;        // The length of time this job has been actively running.
   protected long m_StartTime;      // The time this job was started.
   protected short m_Status;        // The current run status of the job
   ...
   public AceJobProc() 
   {
      ...
      m_Interval = 10 * ProcessServer.minute;
      m_MinStartHour = 0;
      m_MaxStartHour = 23;
      m_MaxRunTime = ProcessServer.hour;
    ...
    * @param name
    */
   public AceJobProc(AceApp app, String name) 
   {
      this();
   ...
   /**
    * The maximum hour (latest start time) allowed for processing to start.
    */
   public long getMaxStartHour() {
      return m_MaxStartHour;
   }

   /**
    * The minimum hour (earliest start time) allowed for processing to start.
    */
   public long getMinStartHour() 
   {
      return m_MinStartHour;
   }
   ...
   /**
    * 
    * @return The return code of any processing.
    */
   public synchronized int getProcessResult() 
   {
      return m_ProcResult;
   }
   ...
   /**
    * This should be overridden by descendant classes.  Note that this should probably still be called
    * after processing is done so the data field is set to null.
    */
         ...
         try {
            while ( m_Status != ProcessServer.stopped ) {
               try {
                  ...
                  if ( hourOfDay >= getMinStartHour() && hourOfDay <= getMaxStartHour() ) {
                  ...
                  if ( ex instanceof InterruptedException ) {
                     if ( m_Status != ProcessServer.idle ) {
                     ...
                     logMsg.append("[AceJobProc] There was an exception during processing.  See ../log/server.log for the stacktrace\r\n");

                     logMsg.append("[AceJobProc] The exception was:\r\n");
    ...
    */
   public void setApplication(AceApp app) throws Exception 
   {
      if ( app != null ) {
         m_App = app;
         ...
         throw new Exception("[AceJobProc] parent application can't be set to null");
   }

   /**
    * Setter for the data that needs to be processed.
    * @param data
    */
    ...
    * 
    * @param interval The time in minutes
    */
    ...
    * 
    * @param maxRunTime The time in minutes.
    */
   ...
   /**
    * Sets the max start hour property.
    * 
    * @param hour The hour in 24 hour time.
    */
   public void setMaxStartHour(int hour) 
   {
      if ( hour >= 0 && hour <= 23 )
         m_MaxStartHour = hour;
   }
   ...
   /**
    * Sets the min start hour property.
    * 
    * @param hour The hour in 24 hour time.
    */
   public void setMinStartHour(int hour) 
   {
      if ( hour >= 0 && hour <= 23 )
         m_MinStartHour = hour;
   }
